...
const emptyDb: AppDatabase = { expenses: [], funds: [], documents: [], materials: [], inventoryTransactions: [] };
...
export default function App() {
  ...
  const updateDb = (updater: (prev: AppDatabase) => AppDatabase, message?: string) => persistDb(updater(db), message);

  const handleResetDB = async () => { if (window.confirm('Khôi phục dữ liệu mẫu?')) await persistDb(seedData, 'Đã khôi phục dữ liệu mẫu.'); };
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-800 flex font-sans antialiased relative">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-y-auto">
        ...
        <div className="md:hidden sticky top-0 z-30 bg-white/90 backdrop-blur border-b border-slate-200 px-3 py-2 print:hidden"><div className="flex gap-2 overflow-x-auto pb-1">{navItems.map((item) => { const Icon = item.icon; const active = activeTab === item.id; return <button key={item.id} onClick={() => setActiveTab(item.id)} className={`shrink-0 inline-flex items-center gap-1.5 px-3 py-2 rounded-xl text-[11px] font-black border ${active ? item.danger ? 'bg-rose-600 text-white border-rose-600' : 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-600 border-slate-200'}`}><Icon className="h-3.5 w-3.5" />{item.short}</button>; })}</div></div>
        <main className="flex-1 p-4 sm:p-6 lg:p-8 space-y-6 print:p-0">
          {loading ? <div className="flex flex-col items-center justify-center p-20 space-y-4"><div className="h-10 w-10 border-4 border-blue-600 border-t-transparent rounded-full animate-spin" /><p className="text-sm font-medium text-slate-500 font-mono">Đang kết nối Google Sheets backend...</p></div> : errorString ? <div className="p-8 bg-rose-50 border border-rose-200 rounded-3xl text-center text-rose-800 space-y-3"><AlertOctagon className="h-12 w-12 text-rose-500 mx-auto" /><p className="font-bold">Đã xảy ra sự cố!</p><p className="text-xs font-mono text-slate-550">{errorString}</p><button onClick={loadData} className="px-4 py-2 bg-rose-600 text-white font-semibold rounded-xl text-xs hover:bg-rose-700">Thử kết nối lại</button></div> : <div className="space-y-6">
