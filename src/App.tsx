...
export default function App() {
  ...
  const handleAddOrUpdateExpense = async (expense: Expense) => {
    updateDb((prev) => {
      ...
      if (index >= 0) nextExpenses[index] = expense;
      else nextExpenses.unshift(expense);
      ...
      <div className="bg-white border border-slate-200 rounded-3xl shadow-sm p-6 space-y-5">
        ...
        <div className="grid gap-4">
          <label className="space-y-2">
            ...
            <input
              ...
              className="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none focus:ring-4 focus:ring-blue-100 focus:border-blue-400"
            />
          </label>
          ...
          <label className="space-y-2">
            ...
            <input
              ...
              placeholder="Mã token đã đặt trong Code.gs"
              className="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none focus:ring-4 focus:ring-blue-100 focus:border-blue-400"
            />
          </label>
        </div>
        ...
        <div className="flex flex-wrap gap-3">
          <button
            ...
            className="inline-flex items-center gap-2 px-5 py-3 rounded-2xl bg-blue-600 text-white text-xs font-extrabold hover:bg-blue-700 disabled:opacity-60"
          >
            <Save className="h-4 w-4" />
            {savingConfig ? 'Đang lưu...' : 'Lưu cấu hình & tải dữ liệu'}
          </button>
          <button
            onClick={loadData}
            className="px-5 py-3 rounded-2xl bg-slate-100 text-slate-700 text-xs font-extrabold hover:bg-slate-200"
          >
            Tải lại dữ liệu
          </button>
        </div>
        ...
        <div className={`p-4 rounded-2xl text-xs font-semibold border ${isBackendConfigured() ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : 'bg-amber-50 border-amber-200 text-amber-800'}`}>
          {isBackendConfigured()
            ? 'Đã có cấu hình backend trong trình duyệt này. Nếu token/URL đúng, dữ liệu sẽ đồng bộ Google Sheets.'
            : 'Chưa cấu hình backend. App vẫn chạy được bằng dữ liệu mẫu/cache nhưng chưa ghi về Google Sheets.'}
        </div>

        <div className="bg-slate-50 rounded-2xl p-4 text-xs text-slate-600 leading-relaxed border border-slate-200">
          <p className="font-bold text-slate-800 mb-2">Các bước bên Google Sheet:</p>
          <p>1. Mở Google Sheet → Extensions → Apps Script.</p>
          <p>2. Copy file <code>apps-script/Code.gs</code> trong repo vào Apps Script.</p>
          <p>3. Đổi <code>SECRET_TOKEN</code>, chạy hàm <code>setupDatabase</code>.</p>
          <p>4. Deploy dạng Web app, quyền truy cập chọn Anyone, rồi copy URL dán vào ô trên.</p>
        </div>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#f8fafc] text-slate-800 flex font-sans antialiased relative">
      {notification && (
        <div className="fixed bottom-6 right-6 z-[100] max-w-sm bg-slate-900/95 backdrop-blur-md border border-slate-800 text-white p-4 rounded-2xl shadow-xl flex items-center gap-3 ring-4 ring-emerald-500/10">
          <span className="flex h-2 w-2 relative shrink-0">
            <span className={`animate-ping absolute inline-flex h-full w-full rounded-full ${notification.type === 'success' ? 'bg-emerald-400' : 'bg-blue-400'} opacity-75`}></span>
            <span className={`relative inline-flex rounded-full h-2 w-2 ${notification.type === 'success' ? 'bg-emerald-500' : 'bg-blue-500'}`}></span>
          </span>
          <div className="flex-1 min-w-0">
            <p className="text-[9px] font-extrabold tracking-widest font-mono uppercase text-emerald-400">ĐỒNG BỘ DỮ LIỆU</p>
            <p className="text-xs text-slate-200 mt-0.5 font-semibold leading-relaxed">{notification.message}</p>
          </div>
          <button onClick={() => setNotification(null)} className="text-slate-400 hover:text-white text-base font-semibold h-6 w-6 rounded-full hover:bg-white/10 flex items-center justify-center">
            &times;
          </button>
        </div>
      )}
      ...
      <aside className="hidden md:flex w-64 bg-slate-900 text-white flex-col border-r border-slate-800 shrink-0 sticky top-0 h-screen print:hidden">
        <div className="p-6 border-b border-slate-800">
          ...
          <p className="text-[10px] text-slate-500 font-mono tracking-wider font-semibold">GitHub Pages + Google Sheets</p>
        </div>
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left text-xs font-semibold tracking-wide transition-all ${
                  active
                    ? item.danger ? 'bg-rose-600 text-white shadow-lg shadow-rose-500/25' : 'bg-blue-600 text-white shadow-lg shadow-blue-500/25'
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-950/20">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full flex items-center justify-center text-xs font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 shrink-0">
              {currentRole === UserRole.ACCOUNTANT ? 'KT' : currentRole === UserRole.STOREKEEPER ? 'TK' : currentRole === UserRole.HCNS ? 'HC' : 'SD'}
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-xs font-bold text-slate-200 truncate leading-none mb-1">{getRoleLabel(currentRole)}</p>
              <div className="flex items-center gap-1">
                <span className={`h-1.5 w-1.5 rounded-full ${syncStatus === 'connected' ? 'bg-emerald-500' : 'bg-amber-500'}`}></span>
                <span className="text-[10px] text-slate-500 italic">{syncStatus === 'connected' ? 'Google Sheets' : 'Cache/local'}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-y-auto">
        <RoleHeader
          ...
          activeUsers={syncStatus === 'connected' ? ['Google Sheets Backend'] : []}
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 space-y-6 print:p-0">
              ...
              {navItems.map((item) => (
                <button
                  ...
                  className={`flex items-center gap-1 px-3 py-2 rounded-xl text-[10px] font-bold transition-all ${
                    activeTab === item.id ? item.danger ? 'bg-rose-600 text-white shadow-xs' : 'bg-blue-600 text-white shadow-xs' : 'text-gray-650 hover:bg-slate-50'
                  }`}
                >
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center p-20 space-y-4">
              ...
              <p className="text-sm font-medium text-slate-500 font-mono">Đang kết nối Google Sheets backend...</p>
            </div>
          ) : errorString ? (
            <div className="p-8 bg-rose-50 border border-rose-200 rounded-3xl text-center text-rose-800 space-y-3">
              ...
              <button onClick={loadData} className="px-4 py-2 bg-rose-600 text-white font-semibold rounded-xl text-xs hover:bg-rose-700">
                Thử kết nối lại
              </button>
            </div>
          ) : (
            <div className="space-y-6">
              <div className="bg-[#f0fdf4] p-4 rounded-xl border border-emerald-200 flex items-start gap-3 text-xs leading-snug text-emerald-800 print:hidden shadow-xs">
                ...
                <div className="space-y-1">
                  <p className="font-bold">Mô hình mới: GitHub Pages + Google Sheets</p>
                  <p className="text-emerald-700">
                    Giao diện chạy trên GitHub Pages. Dữ liệu đọc/ghi qua Apps Script vào Google Sheets. Nếu chưa cấu hình backend, hệ thống vẫn chạy bằng dữ liệu mẫu/cache để kiểm thử.
                  </p>
                </div>
              </div>
        ...
        <footer className="py-6 border-t border-slate-200 mt-auto bg-white/40 text-center text-[10px] text-slate-400 font-medium print:hidden">
          Hệ thống kế toán dự án © 2026. Frontend GitHub Pages, backend Google Sheets/Apps Script.
        </footer>
      </div>
    </div>
  );
